...
from tensorflow.keras.layers import Dense, BatchNormalization, Reshape, Activation, Conv2D, ZeroPadding2D, Add, Input, MaxPooling2D, GlobalAveragePooling2D
...
def ResNet(model_params, input_shape=None, input_tensor=None, include_top=True,
    ...
    if len(input_shape) == 2:
        x = Reshape((-1, input_shape[1], 1))(img_input)
    else:
        x = img_input

    # Normalize along coeffients and time
    x = BatchNormalization(name='bn_data', **no_scale_bn_params)(x)
    ...
    # resnet top
    if include_top:
        x1 = GlobalAveragePooling2D(name='pool1')(x)
        x = Dense(classes, name='fc1')(x1)
...
MODELS_PARAMS = {
    'resnet10': ModelParams('resnet18', (2, 2), residual_conv_block, None),
